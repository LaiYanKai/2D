...
algs = ["R2", "TS2B"]; % make sure the first alg returns the optimal path, and the optimal path does not contain more than 3 consecutive colinear points anywhere
expt_nums = 0:9;
map_pairs = [
    "dao", "arena_scale2";
...
SU = cell2table(SU, "RowNames", T_nsecs.Properties.RowNames, ...
    "VariableNames", ["unique_points", "R2_ANYA", "R2_RSP"]);
...
for m = 1:height(map_pairs)
    ...
    semilogy(unique_points, SU_R2_ANYA{:}, '.-');
    hold on
    semilogy(unique_points, SU_R2_RSP{:}, 'x-');
    yline(1, ':');
    legend(["R2 vs ANYA", "R2 vs RayScan+"], 'Location', 'north' );
    ylim([0.1, inf])
    grid on
    if m == 1
...
for m = 1:height(T)
    ...
    for p = P
        ...
        idx = unique_points == p;
        su = [SU.R2_ANYA{m}(idx), SU.R2_RSP{m}(idx)];
        for s = su
            if isempty(s)
                fprintf(" & --");
            else
...
%% for Yk's use %% Before merging R2E to R2.
% algs = ["R2E", "TS2B"]; % make sure the first alg returns the optimal path, and the optimal path does not contain more than 3 consecutive colinear points anywhere
% expt_nums = 0:9;
% map_pairs = [
%     "dao", "arena_scale2";
%         "bg512", "AR0709SR_scale2";
...
% end
% SU = cell2table(SU, "RowNames", T_nsecs.Properties.RowNames, ...
%     "VariableNames", ["unique_points", "R2E_ANYA", "R2E_RSP"]);
...
%         su = [SU.R2E_ANYA{m}(idx), SU.R2E_RSP{m}(idx)];
%         for s = su
%             if isempty(s)
%                 fprintf(" & --");
%             else
