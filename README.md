...
Primarily features novel algorithms like R2 and R2E. 
...
**R2/R2E**: An optimal, novel vector-based any-angle algorithm. Delays line-of-sight checks and expands only the most promising turning points to return paths quickly on large sparse maps, or maps with few disjoint obstacles. R2E is faster for maps with many disjoint obstacles due to cost-to-come verifications. 
...
MATLAB scripts are available in the P2D library to generate the scaled maps, show plots for debugging and generate results for the R2/R2E paper. As they are for internal use, they will not be described.
...
**--algs**: At least one of `ANYA2B`, `ANYA2N`, `R2`, `R2E`, `TS2B`, `TS2N`, `VG2B`, `VG2N` must be specified.
...
## Run
An example to run with `R2` and `R2E` on the scaled map `arena_scale2` used in the R2/R2E paper is available in `run.sh` and `.vscode/launch.json`. 
...
Suppose `VG2B`, `R2` and `R2E` are to be tested on all scenarios for the files:
...
The parameters should be: `--algs VG2B R2 R2E --result_dir results --map_dir data --scen_dir data --names da2/ht_mansion2b dao/arena`.
The result files are:
- results/da2/ht_mansion2b.VG2B.results
- results/da2/ht_mansion2b.R2.results
- results/da2/ht_mansion2b.R2E.results
- results/dao/arena.VG2B.results
- results/dao/arena.R2.results
- results/dao/arena.R2E.results

### Test All Scenarios Multiple Times
This example is useful for obtaining average run times for each scenario.
Suppose `R2E` and `R2` are to be tested on all scenarios three times for the files:
- data/sc1/Aftershock.map.scen
- data/sc1/Aftershock.map

and the result files stored in the `results` folder in their respective sub-folders.

The parameters are: `--algs R2E R2 --result_dir results --map_dir data --scen_dir data --names sc1/Aftershock --num_expts 3`. The results files are:
- results/sc1/Aftershock.R2E.0.results
- results/sc1/Aftershock.R2.0.results
- results/sc1/Aftershock.R2E.1.results
- results/sc1/Aftershock.R2.1.results
- results/sc1/Aftershock.R2E.2.results
...
Suppose `R2E` is to be run for scenario ids 5 and 100:
- Aftershock.map.scen
- Aftershock.map

The parameters are: `--algs R2E --names Aftershock --ids 5 100`. The results are printed onto the terminal.
...
# Verbosity
Verbosity is supported for `R2`, `R2E` and `ANYA2B` and `ANYA2N`, where the expansions are printed to the terminal and a log file.
...
For example, to use *R2E* and *VG2B*, link the libraries in your folder's CMakeLists.txt (see [run/CMakeLists.txt](https://github.com/LaiYanKai/2D/blob/main/run/CMakeLists.txt)):
```CMake
...
To initialise *R2E*:
```cpp
P2D::R2::R2 r2(&grid, true);
```
where `true` is for *R2E* and `false` is for *R2*.
